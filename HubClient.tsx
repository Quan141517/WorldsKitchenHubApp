...
export function HubClient({ session, initialData }: { session: DiscordSession; initialData: HubData }) {
  ...
  const copyProtected = Boolean(role && role.level < 100);
...
function Header({ session, role, profile, saveRobloxUsername }: { session: DiscordSession; role: StaffRole | null; profile?: StaffProfile; saveRobloxUsername: (robloxUsername: string) => Promise<void> }) {
  const [profileOpen, setProfileOpen] = useState(false);
  const [robloxUsername, setRobloxUsername] = useState(profile?.robloxUsername || "");
  const profileAvatarUrl = profile?.robloxAvatarUrl || session.avatarUrl;

  useEffect(() => {
    setRobloxUsername(profile?.robloxUsername || "");
  }, [profile?.robloxUsername]);

  async function submitRobloxUsername(event: React.FormEvent) {
    event.preventDefault();
    if (!robloxUsername.trim()) return;
    await saveRobloxUsername(robloxUsername.trim());
  ...
  return (
    <header className="topbar">
      ...
      {profileOpen ? (
        <form className="profile-popover" onSubmit={submitRobloxUsername}>
          <label>Roblox username<input value={robloxUsername} onChange={(event) => setRobloxUsername(event.target.value)} placeholder="Roblox username" /></label>
          <p className="muted">{profile?.robloxUserId ? `Linked to Roblox user ID ${profile.robloxUserId}.` : "Enter your Roblox username to link your profile and avatar."}</p>
          <div className="dialog-actions">
            <button className="button secondary" type="button" onClick={() => setProfileOpen(false)}>Cancel</button>
