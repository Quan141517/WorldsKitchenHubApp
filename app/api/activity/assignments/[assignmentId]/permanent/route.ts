...
export async function DELETE(_request: NextRequest, context: { params: Promise<{ assignmentId: string }> }) {
  ...
  const data = await updateHubData((hubData) => {
    ...
    const index = hubData.weeklyAssignments.findIndex((assignment) => assignment.id === assignmentId);
    if (index === -1) return hubData;

    const [removed] = hubData.weeklyAssignments.splice(index, 1);
    deleted = true;
    addAuditLog(hubData, {
      action: "Weekly assignment permanently deleted",
      detail: removed.teamRoleId,
